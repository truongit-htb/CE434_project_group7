...
def write_weights(file_b,file_k, file_in, block):
    for listweights in list(file_in["model_weights"]["vgg16"][block]["bias:0"]):
        file_b.writelines('%s\n' % listweights)
    for listweights in list(file_in["model_weights"]["vgg16"][block]["kernel:0"]):
        for i in range(2):
            if(i==0):
                file_k.writelines("Channel B\n")
                file_k.writelines('%s\n' % listweights[0])
            if(i==1):
                file_k.writelines("Channel G\n")
                file_k.writelines('%s\n' % listweights[1])
            if(i==2):
                file_k.writelines("Channel R\n")
                file_k.writelines('%s\n' % listweights[2])
